...
class Airport():
    ...
    def load_airports():
        for line in data.split('\n'):
            Airport(*line.strip().split("\t")) # Δημιουργούμε 5 Airport objects
    ...
    def __init__(self, code, name, coordinates):
        ...
        Airport.airport_dict[code] = self # Λεξικό με το code σαν key και το Object σαν value
...
class Trip():
    ...
    def __init__(self, itinerary):
        self.itinerary = itinerary # String of airports e.g 'KLX-ATH-RHO-ATH'
        self.airports = itinerary.split('-') # Χωρίζουμε το string στην παύλα
        self.total_distance = 0 # Αρχική απόσταση 0
    ...
    def __str__(self):
        ...
        for code_num in range(len(self.airports) - 1):
            #print('Hey')
            dist = Airport.airport_dict[self.airports[code_num]].get_distance(Airport.airport_dict[self.airports[code_num + 1]])
            print(f'<{Airport.airport_dict[self.airports[code_num]].name} - {Airport.airport_dict[self.airports[code_num + 1]].name}> \
                  {dist}')
            self.total_distance +=  dist
        return f'Total distance: {self.total_distance}'
